...
plot.robustEM <- function(x, ...){
  ...
  plot.new()
  ellipses <- lapply(1:nrow(meanvec), function(j) mixtools::ellipse(mu=x$mu[j,],
  ...
  covar_plot_f_EM <- function(j){
    ...
    return(ggplot2::geom_path(data = boundary, aes(x=Axis1, y=Axis2), size=0.5, color = 'red'))
  }

  covar_EM_plots <- lapply(1:nrow(x$mu), covar_plot_f_EM)
  cluster_num = cut(apply(x$T_mat,2,which.max),c(1:x$cluster))
  final_plot <- ggplot() +
    geom_point(data = cluster_result, aes(x=Axis1, y=Axis2, color = cluster_num), size = 1.5, alpha = 0.5) +
    geom_point(data = meanvec, aes(x=Axis1, y=Axis2), shape=23, size = 3, stroke = 2, color = 'red') +
    covar_EM_plots+theme(legend.position="none")
